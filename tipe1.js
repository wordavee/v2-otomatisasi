(() => {
  ...
  function renderPreview() {
    ...
    ui.fileName.textContent = `${String(state.previewIndex + 1).padStart(3, "0")}-tipe-1.mp4`;
  }
  ...
  function renderTemplate(context, width, height, quote) {
    ...
    const maxWidth = width * .825;
    const lineHeightRatio = 1.34;
    let fontSize = 174 * scale;
    ...
    while (fontSize >= minFontSize) {
      ...
      if (lines.length <= 7 && blockHeight <= height * .55) break;
      fontSize -= 3 * scale;
    }
    ...
    context.font = `700 ${107 * scale}px Arial, Helvetica, sans-serif`;
  ...
  async function generateAll() {
    ...
    try {
      const result = await TemplateVideo.exportBatch({
        ...
        zipName: `video-tipe-1-${total}-${date}.zip`,
      ...
      await delay(450);
      closeProgress();
      showToast(`${total} video Tipe 1 berhasil dibuat dalam format ${result.extension.toUpperCase()}.`);
    } catch (error) {
      console.error(error);
      closeProgress();
  ...
  function makeVideoBaseName(index, quote) {
    ...
    return `${number}-tipe-1-${slug || "quotes"}`;
  }
